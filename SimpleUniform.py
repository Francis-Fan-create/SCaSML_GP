...
class SimpleUniform(object):
    ...
    def test(self, save_path, rhomax=2, num_domain=1000, num_boundary=200):
        ...
        start = time.time()
        x_domain = data_domain[:, :self.dim]  # x_domain: ndarray, shape: (num_domain, dim), dtype: float
        x_boundary = data_boundary[:, :self.dim]  # X_boundary: ndarray, shape: (num_boundary, dim), dtype: float
        sol1 = self.solver1.GPsolver(x_domain, x_boundary, GN_step=4)  # sol1: ndarray, shape: (num_domain,), dtype: float
