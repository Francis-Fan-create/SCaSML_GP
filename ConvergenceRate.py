...
class ConvergenceRate(object):
    ...
    def test(self, save_path, rhomax=5, n_samples=5):
        ...
        x_boundary = data_boundary[:, :-1]  # x_boundary: ndarray, shape: (num_boundary, dim), dtype: float
        ...
        for random_method in random_methods:
            ...
            plt.figure()
            plt.plot(np.log10(evaluation_number_array1 + epsilon), np.log10(np.array(errors1_array) + epsilon), label='GP')
            ...
            plt.title(f'GP - Convergence Rate {random_method}')
            plt.xlabel('log10(evaluation_number)')
            plt.ylabel('log10(error)')
            plt.legend()
            plt.savefig(f'{save_path}/GP_convergence_rate_{random_method}.png')
            wandb.log({f'GP_convergence_rate_{random_method}': plt})
