...
#initialize wandb
wandb.init(project="Grad_Dependent_Nonlinear", notes="45 d", tags=["Gaussian Process"],mode="disabled") #debug mode
# wandb.init(project="Grad_Dependent_Nonlinear", notes="45 d", tags=["Gaussian Process"]) #working mode
wandb.config.update({"device": device.type}) # record device type

#initialize the equation
equation=Grad_Dependent_Nonlinear(n_input=46,n_output=1)
...
# rhomax=test1.test(r"results_full_history/Grad_Dependent_Nonlinear/45d")
#run the test for SimpleUniform
test2=SimpleUniform(equation,solver1,solver2,solver3)
test2.test(r"results_full_history/Grad_Dependent_Nonlinear/45d")
#run the test for ConvergenceRate
test3=ConvergenceRate(equation,solver1,solver2,solver3)
test3.test(r"results_full_history/Grad_Dependent_Nonlinear/45d")
