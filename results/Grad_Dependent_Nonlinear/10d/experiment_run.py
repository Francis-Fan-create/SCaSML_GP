...
#initialize wandb
wandb.init(project="Grad_Dependent_Nonlinear", notes="10 d", tags=["Gaussian Process"],mode="disabled") #debug mode
# wandb.init(project="Grad_Dependent_Nonlinear", notes="10 d", tags=["Gaussian Process"]) #working mode
wandb.config.update({"device": device.type}) # record device type

#initialize the equation
equation=Grad_Dependent_Nonlinear(n_input=10,n_output=1)
...
# rhomax=test1.test(r"results/Grad_Dependent_Nonlinear/10d")
#run the test for SimpleUniform
test2=SimpleUniform(equation,solver1,solver2,solver3)
test2.test(r"results/Grad_Dependent_Nonlinear/10d")
#run the test for ConvergenceRate
test3=ConvergenceRate(equation,solver1,solver2,solver3)
test3.test(r"results/Grad_Dependent_Nonlinear/10d")
#run the test for GPRate
test4=GPRate(equation,solver1)
test4.test(r"results/Grad_Dependent_Nonlinear/10d")
